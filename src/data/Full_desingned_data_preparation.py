...
os.chdir(directory)
# ---set options for maximum rows and columns
pd.set_option('display.max_columns', None)


# ---Import both train and test sets
titanic_train_data = pd.read_csv(
    directory+"/train.csv", header=0, encoding="utf-8")
titanic_test_data = pd.read_csv(
    directory+"/test.csv", header=0, encoding="utf-8")
...
titanic_train_data['family_size'] = titanic_train_data['SibSp'] + \
    titanic_train_data['Parch'] + 1
titanic_test_data['family_size'] = titanic_test_data['SibSp'] + \
    titanic_test_data['Parch'] + 1
...
print(titanic_train_data[['is_alone', 'Survived']].groupby(
    ['is_alone'], as_index=False).mean())
print(titanic_train_data[['family_size', 'Survived']].groupby(
    ['family_size'], as_index=False).mean())
...
titanic_train_data['title'] = titanic_train_data['title'].replace(
...
titanic_test_data['title'] = titanic_test_data['title'].replace(
    ['Lady', 'Countess', 'Capt', 'Col', 'Don', 'Dr', 'Major', 'Rev', 'Sir', 'Jonkheer', 'Dona'], 'Rare')
...
# Prob. of survived depending on the title of each one
print("----------------------")
print(titanic_train_data[['title', 'Survived']].groupby(
...
# Prob. of survived depending on the Fare category
print(titanic_train_data[["category_fare", "Survived"]].groupby(
    ["category_fare"], as_index=False).mean())

# Mapping Fare based on the binnning implemented above
# create new identical feature to avoid confusions
titanic_train_data['Fare_new'] = titanic_train_data['Fare']
titanic_test_data['Fare_new'] = titanic_test_data['Fare']  # >>

titanic_train_data.loc[titanic_train_data['Fare'] <= 7.91, 'Fare_new'] = 0
titanic_train_data.loc[(titanic_train_data['Fare'] > 7.91) & (
    titanic_train_data['Fare'] <= 14.454), 'Fare_new'] = 1
titanic_train_data.loc[(titanic_train_data['Fare'] > 14.454) & (
    titanic_train_data['Fare'] <= 31), 'Fare_new'] = 2
...
titanic_test_data.loc[(titanic_test_data['Fare'] > 7.91) & (
    titanic_test_data['Fare'] <= 14.454), 'Fare_new'] = 1
titanic_test_data.loc[(titanic_test_data['Fare'] > 14.454) & (
    titanic_test_data['Fare'] <= 31), 'Fare_new'] = 2
...
random_list_train = np.random.randint(
    age_avg - age_std, age_avg + age_std, size=age_null_train)
random_list_test = np.random.randint(
    age_avg - age_std, age_avg + age_std, size=age_null_test)

# Age categories (replace missing values of Age)

titanic_train_data['Age_cat'] = titanic_train_data['Age']
titanic_train_data['Age_cat'][np.isnan(
    titanic_train_data['Age'])] = random_list_train
titanic_train_data['Age_cat'] = titanic_train_data['Age_cat'].astype(int)

titanic_test_data['Age_cat'] = titanic_test_data['Age']
titanic_test_data['Age_cat'][np.isnan(
    titanic_test_data['Age'])] = random_list_test
...
# Prob. of survived depending on the Age category
print(titanic_train_data[["category_age", "Survived"]].groupby(
    ["category_age"], as_index=False).mean())

# Mapping Age based on the binnning implemented above
# create new identical feature to avoid confusion
titanic_train_data['Age_new'] = titanic_train_data['Age']
titanic_test_data['Age_new'] = titanic_test_data['Age']  # >>

titanic_train_data.loc[titanic_train_data['Age'] <= 16, 'Age_new'] = 0
titanic_train_data.loc[(titanic_train_data['Age'] > 16) & (
    titanic_train_data['Age'] <= 32), 'Age_new'] = 1
titanic_train_data.loc[(titanic_train_data['Age'] > 32) & (
    titanic_train_data['Age'] <= 48), 'Age_new'] = 2
titanic_train_data.loc[(titanic_train_data['Age'] > 48) & (
    titanic_train_data['Age'] <= 64), 'Age_new'] = 3
titanic_train_data.loc[titanic_train_data['Age'] > 64, 'Age_new'] = 4

titanic_test_data.loc[titanic_test_data['Age'] <= 16, 'Age_new'] = 0
titanic_test_data.loc[(titanic_test_data['Age'] > 16) & (
    titanic_test_data['Age'] <= 32), 'Age_new'] = 1
titanic_test_data.loc[(titanic_test_data['Age'] > 32) & (
    titanic_test_data['Age'] <= 48), 'Age_new'] = 2
titanic_test_data.loc[(titanic_test_data['Age'] > 48) & (
    titanic_test_data['Age'] <= 64), 'Age_new'] = 3
titanic_test_data.loc[titanic_test_data['Age'] > 64, 'Age_new'] = 4
# Prob. of survived depending on the Age category (same as before)
print(titanic_train_data[['Age_new', "Survived"]].groupby(
    ['Age_new'], as_index=False).mean())


# Embarked-Survived
print(titanic_train_data[["Embarked", "Survived"]].groupby(
    ["Embarked"], as_index=False).mean())
# Pclass-Survived
print(titanic_train_data[["Pclass", "Survived"]].groupby(
    ["Pclass"], as_index=False).mean())
# -- Dropping of several useless variables for both train and test datasets
columns_to_drop = ["PassengerId", "Ticket", "Cabin", "Name", "SibSp",
                   "Parch", "family_size", "category_fare", "category_age", "Age_cat"]
...
# age and embarked include missing values
titanic_train_data.isna().sum().sort_values(ascending=False)
titanic_test_data.isna().sum().sort_values(ascending=False)  # age and fare >>

# Implement exploratory data analysis (EDA) to understand the patterns in our data
#exec(open("C:/Users/vasileios palaskas/Desktop/Competitions/Structured_crossval/visualisation_descriptive_analysis.py").read())

...
outer_yhat_bayessearach_ann =list()
outer_yhat_train_bayessearach_ann =list()
...
for train_ix, test_ix in cv_outer.split(X,y):
    ...
    outer_yhat_bayessearach_ann.append(yhat)
    outer_yhat_train_bayessearach_ann.append(yhat_train)
...
print('Out-Sample Accuracy: %.3f (%.3f)' % (np.mean(outer_results), np.std(outer_results))) #Accuracy:81.100 (3.176)
print('Out-Sample F1 score: %.3f (%.3f)' % (np.mean(outer_results_f1), np.std(outer_results_f1))) #F1:73.100 (4.158)

print('In-Sample Accuracy: %.3f (%.3f)' % (np.mean(in_sample_acc_results), np.std(in_sample_acc_results))) #Accuracy: 85.600 (0.800)
print('In-Sample F1 score: %.3f (%.3f)' % (np.mean(in_sample_f1_results), np.std(in_sample_f1_results))) #F1:79.600 (1.562)

#%%%% Save model training and model prediction results both inner and outer cv splits 
with open("outer_results_bayessearach_ann", "wb") as outer_results_fp:   #Pickling
 pickle.dump(outer_results, outer_results_fp)

with open("outer_results_bayessearach_ann", "rb") as outer_results_fp:   # Unpickling
 outer_results_bayessearach_ann = pickle.load(outer_results_fp)
#
 with open("in_sample_acc_results_bayessearach_ann", "wb") as in_sample_acc_results_fp:   #Pickling
  pickle.dump(in_sample_acc_results, in_sample_acc_results_fp)

 with open("in_sample_acc_results_bayessearach_ann", "rb") as in_sample_acc_results_fp:   # Unpickling
  in_sample_acc_results_bayessearach_ann = pickle.load(in_sample_acc_results_fp)
 #
 with open("outer_yhat_bayessearach_ann", "wb") as outer_yhat_bayessearach_ann_fp:   #Pickling
  pickle.dump(outer_yhat_bayessearach_ann, outer_yhat_bayessearach_ann_fp)
with open("outer_yhat_bayessearach_ann", "rb") as outer_yhat_bayessearach_ann_fp:   # Unpickling
        yhat_bayessearach_ann_list = pickle.load(outer_yhat_bayessearach_ann_fp)
#%%%% Store our predictions (saved in models/RF_Predictions)

with open("y_out_sample_test_results_bayessearach_ann", "wb") as y_out_sample_test_results_fp:   #Pickling
 pickle.dump(y_out_sample_test_results, y_out_sample_test_results_fp)

with open("y_out_sample_test_results_bayessearach_ann", "rb") as y_out_sample_test_results_fp:   # Unpickling
 y_out_sample_test_results_bayessearach_ann_list = pickle.load(y_out_sample_test_results_fp) 

         
 #%%%% Store the best params in each outer cross-validation trial
 with open("inner_best_params_bayessearach_ann", "wb") as inner_best_params_bayessearach_ann:   #Pickling
     pickle.dump(inner_best_params, inner_best_params_bayessearach_ann)

 with open("inner_best_params_bayessearach_ann", "rb") as inner_best_params_bayessearach_ann:   # Unpickling
    inner_best_params = pickle.load(inner_best_params_bayessearach_ann) 
